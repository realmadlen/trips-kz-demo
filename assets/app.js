...
window.App = (function () {
  ...
  /* Колесо мыши над лентами не перехватывается. Такой перехват здесь был:
     он останавливал событие и прокручивал страницу вручную, и вместе с
     плавной прокруткой из base.css это давало рывки и подвисания — каждый
     щелчок колеса запускал новую анимацию поверх незакончившейся. Вертикаль
     ленты закрыта через overflow-y: hidden, этого достаточно: странице
     жест достаётся сам, без нашей помощи. */
  ...
  function tourRow(tour, opts) {
    ...
    return '<article class="tour-row reveal" data-tour="' + tour.id + '" tabindex="0" ' +
      ...
      '<div class="tour-row__price">' +
        ...
        /* «Купить тур» ничего не покупало: открывалась форма заявки —
           кнопка называет то, что произойдёт. */
