...
window.App = (function () {
  ...
  function guestsLabel(prm) {
    /* Малыши до двух лет считаются отдельной строкой: «3 реб.» и «2 реб. ·
       1 до 2 лет» — это разные заявки и разная цена. */
    var inf = prm.kids.filter(function (a) { return a < 2; }).length;
    var kid = prm.kids.length - inf;
    var s = prm.adults + ' ' + t('g.adults');
    if (kid) s += ' · ' + kid + ' ' + t('g.kids');
    if (inf) s += ' · ' + inf + ' ' + t('g.infants');
    return s;
  }
  ...
  /* Любая лента (карусель фото в карточке, похожие туры, блог, команда) —
     это блок с горизонтальной прокруткой. Курсор над ним, колесо вниз — и
     браузер отдаёт вертикальный жест единственной оси, которая у блока есть:
     страница стоит, а лента едет вбок. Ждут ровно обратного, поэтому чисто
     вертикальное колесо всегда достаётся странице, а ленте остаётся то, что
     ей и адресовано, — горизонтальный жест трекпада и палец. */
  var SCROLLERS = '.shots__track, .rail, .team, .row--slider, .operators';
  function tameWheel() {
    document.addEventListener('wheel', function (e) {
      if (Math.abs(e.deltaX) > Math.abs(e.deltaY)) return;
      if (!e.target || !e.target.closest || !e.target.closest(SCROLLERS)) return;
      e.preventDefault();
  ...
  /* Выпадашка привязана к своему полю (left: 0), и у правого поля на узком
     экране она уезжала за край: страница получала 14 лишних пикселей ширины
     и начинала болтаться вбок. Сдвигаем ровно на столько, на сколько вылезла. */
  function fitPop(pop) {
    if (!pop) return;
    pop.style.transform = '';
    var edge = document.documentElement.clientWidth - 8;
    var over = pop.getBoundingClientRect().right - edge;
  ...
  /* Гости. Дети разведены на две строки не для красоты: до двух лет ребёнок
     летит на руках и без собственного места, и это меняет и цену, и то, какой
     номер вообще подойдёт. Одна строка «Дети» с выпадающим возрастом это
     скрывала — возраст выбирали уже после того, как посмотрели цену.
     Условие тарифа вынесено в само название строки: «Дети от 2 до 17 лет» и
     «Малыши до 2 лет» — читается за один взгляд и не требует сноски. */
  SearchForm.prototype.renderGuests = function (field) {
    var self = this, p = this.p;
    var pop = qs('[data-guests]', field);
    var INFANT_AGE = 1;   /* «до 2 лет» хранится в тех же годах, что и остальные */
    var KID_AGE = 7;      /* возраст по умолчанию у ребёнка со своим местом */

    /* Дети и малыши лежат в одном массиве p.kids — так их видит и ссылка,
       и выдача. Разбор по возрасту нужен только здесь, для двух строк. */
    var kidIdx = [], infIdx = [];
    p.kids.forEach(function (age, i) { (age < 2 ? infIdx : kidIdx).push(i); });
    var MAX_KIDS = 4;
    var freeSlots = MAX_KIDS - p.kids.length;

    function row(label, value, key, min, max) {
      return '<div class="stepper"><span class="stepper__label">' + esc(label) + '</span>' +
        '<span class="stepper__ctrl">' +
    ...
    /* Возраст спрашивается только у тех, у кого он влияет на цену: у малышей
       до двух лет выбирать нечего, и лишний селект там был бы шумом. */
    var kidsAges = kidIdx.map(function (idx, n) {
      return '<label class="check"><span>' + esc(t('f.kidAge')) + ' ' + (n + 1) + '</span></label>' +
        '<select class="select" data-kid="' + idx + '" aria-label="' + esc(t('f.kidAge')) + '">' +
          Array.from({ length: 16 }, function (_, k) {
            var a = k + 2;
            return '<option value="' + a + '"' + (a === p.kids[idx] ? ' selected' : '') + '>' + a + '</option>';
          }).join('') +
        '</select>';
    }).join('');
    ...
    pop.innerHTML =
      ...
      row(t('f.kids'), kidIdx.length, 'kids', 0, kidIdx.length + Math.max(0, freeSlots)) +
      (kidIdx.length ? '<div class="pop__group">' + kidsAges + '</div>' : '') +
      row(t('f.infants'), infIdx.length, 'infants', 0,
          Math.min(2, infIdx.length + Math.max(0, freeSlots)));
    ...
    qsa('[data-inc], [data-dec]', pop).forEach(function (b) {
      b.addEventListener('click', function () {
        ...
        if (key === 'adults') p.adults = Math.max(1, Math.min(6, p.adults + (inc ? 1 : -1)));
          ...
          var list = key === 'infants' ? infIdx : kidIdx;
          if (inc && p.kids.length < MAX_KIDS) p.kids.push(key === 'infants' ? INFANT_AGE : KID_AGE);
          if (!inc && list.length) p.kids.splice(list[list.length - 1], 1);
        }
